...
import pl.edu.agh.wmazur.avs.http.flows.SimulationStateDeltaParser.LastStateProvider.UpdateState
...
object SimulationStateDeltaParser {
      ...
      GraphDSL.create() { implicit builder =>
        ...
        val lastState = builder.add {
          Flow[SimulationState]
            .prepend {
              Source.single(SimulationState.empty)
            }
        }
  ...
  object LastStateProvider {
    ...
    def persist(state: SimulationState = SimulationState.empty)
