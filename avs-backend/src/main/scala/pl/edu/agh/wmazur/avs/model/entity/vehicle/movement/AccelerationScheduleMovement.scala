  ...
  private def internalMoveWithSchedule(currentTime: Timestamp,
    ...
    if (events.isEmpty) {
      ...
      current match {
        ...
        case event @ AccelerationTimestamp(acc, timeStart, timeEnd)
            if timeStart < currentTime =>
//          if (acceleration.isEqual(acc) || acc.isZero) {
